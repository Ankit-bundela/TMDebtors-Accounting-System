...
def getCustomers(user=Depends(get_current_user)):   
    ...
    except Exception as e:raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,detail="Internal Server Error")


@router.post('/addCustomer', status_code=201)
def postAddCustomer(user=Depends(get_current_user),
    name: str = Form(...),
    address: str = Form(...),
...
def postUpdateCustomer(user=Depends(get_current_user),
...
def postRemoveCustomer(code: int = Form(...),user=Depends(get_current_user)):
    manager = CustomerManager()
